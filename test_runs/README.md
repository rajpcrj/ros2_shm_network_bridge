...
The bridge's CPU line is **nearly flat** (~1.2 %/subscriber) while every DDS
transport climbs **~8× steeper** (~10 %/subscriber). At N=64 the bridge raw path is
at **78 % of one core**; FastDDS / ros2_shm_msgs are at **~620–670 %** (~6–7 cores).
This is the futex payoff: readers sleep until woken, so fan-out cost barely grows.
